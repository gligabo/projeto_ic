...
Uma variação mais eficiente é o FISTA (\textit{Fast Iterative Shrinkage-Thresholding Algorithm}) \cite{bet09}, que utiliza o momento de Nesterov para acelerar a convergência.
...
\end{aligned}
\]
Neste caso, não existe forma fechada para o cálculo do operador proximal, mas o algoritmo rápido apresentado em \cite{bet09b} será implementado na linguagem python. 
\section{Resultados}
...
Em todos os experimentos desse tipo de problema, utilizou-se a avaliação do valor da função objetivo $f$ como critério de parada (visto que não há adição de ruído nas aquisições das imagens, sabemos que o mínimo da função é $0$). Como operador de convolução $A$, foi adotado um kernel de média de dimensões $7 \times 7$. Em um experimento inicial, utilizou-se um limite máximo de 1000 iterações como critério de parada e uma tolerância de $1.0 \times 10^{-2}$.
...
Para valores elevados de $\gamma$, o operador de soft-thresholding impõe uma forte esparsidade à solução, limitando a magnitude dos iterados. No ISTA, esse efeito favorece a estabilidade. Como cada iteração $x_k$ depende exclusivamente de $x_{k-1}$, o soft-thresholding atua como um amortecedor, mantendo os iterados próximos, como visto na Figura \ref{fig:norm_diff} e, consequentemente, o gradiente de $f$ varia pouco, já que este é Lipschitz contínuo. Com gradientes semelhantes, a busca linear exata produz tamanhos de passo $\lambda_k$ que se estabilizam, permitindo a convergência observada na Figura \ref{fig:passos_gamma_grande}. Já no FISTA, a instabilidade persiste devido ao termo de \textit{momentum}. A extrapolação $y_k = x_k + \beta_k(x_k - x_{k-1})$ amplifica pequenas diferenças entre iterados. O gradiente $\nabla f(y_k)$ pode diferir significativamente entre iterações, levando a passos $\lambda_k$ distintos e reverberando oscilações que o soft-thresholding não consegue suprimir. Esse comportamento é evidenciado nas Figuras \ref{fig:norm_diff} e \ref{fig:passos_gamma_grande}, onde as curvas do FISTA não apresentam a estabilização observada no ISTA.

Para os valores intermediários, que são os de grande interesse já que resultam nas melhores reconstruções das imagens (ver Tabela \ref{tab:psnr_dif_gammas}), o operador de \textit{soft thresholding} não consegue amortecer as oscilações da busca exata. Sendo assim, tanto o ISTA quanto o FISTA apresentam instabilidades.


\begin{table}[H]
    \centering
    \caption{PSNR para diferentes valores de $\gamma$}
    \label{tab:psnr_dif_gammas}
    \begin{tabular}{lc}
        \toprule
        \textbf{Gamma} & \textbf{PSNR}\\
        \midrule
        $0.0001$ & 27.0 dB\\
        $0.001$ & 27.87 dB\\
        $0.01$ & 25.64 dB\\
...
\vspace{0.2cm}
{\footnotesize Aqui foi considerado as reconstruções do FISTA com \textit{backtracking}}
