...
def phantom( shape = ( 256, 256 ), desc = None, top_left = ( -1.0, 1.0 ), bottom_right = ( 1.0, -1.0 ) ):

   if len( shape ) != 2:
      raise ValueError( 'shape must be bidimensional' )
   ...
   x_sampling_dist = ( bottom_right[ 0 ] - top_left[ 0 ] ) / ( shape[ 1 ] - 1 );
   y_sampling_dist = ( bottom_right[ 1 ] - top_left[ 1 ] ) / ( shape[ 0 ] - 1 );
   ...
   for i in range( desc.shape[ 0 ] ):
      ...
      x = top_left[ 0 ] + numpy.tile( numpy.arange( shape[ 1 ] ), ( shape[ 0 ], 1 ) ) * x_sampling_dist - x0
      y = top_left[ 1 ] + numpy.tile( numpy.arange( shape[ 0 ] ), ( shape[ 1 ], 1 ) ).T * y_sampling_dist - y0
      val = (
               ( ( ( y * s ) + ( x * c ) ) / a ) ** 2.0 + \
               ( ( ( y * c ) - ( x * s ) ) / b ) ** 2.0
            ) ** 0.5
